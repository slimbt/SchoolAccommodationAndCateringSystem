...
export class RestaurationParentService {
  Url = 'http://localhost:8080/restaurations'
  idParent: any;
  ...
  updateRestauration(hebergement: any) {
   
    return this.httpp.put(`${this.Url}/${hebergement.id}`, hebergement);
  }
  addRestauration(hebergementData: any){
...
getRestaurationByParentId(idParent: any) {
  return this.httpp.get<any[]>(`${this.Url}/restauration/${idParent}`); // Endpoint pour récupérer les feedbacks par ID du parent
}
getRestaurationByEleveId(idEleve: any) {
  return this.httpp.get<any[]>(`${this.Url}/restaurationn/${idEleve}`); // Endpoint pour récupérer les feedbacks par ID du parent
}

getNumberOfRestauration(): Observable<number> {
  return this.httpp.get<number>(`${this.Url}/count`);
}

getNumberOfRestaurationByCre(): Observable<any[]> {
  return this.httpp.get<any[]>(`${this.Url}/countByCree`);
}
}